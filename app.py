...
def auth_gate():
    ...
    if not stored:
        ...
        a = st.text_input("새 비밀번호 (4자리 이상 숫자)", type="password")
        b = st.text_input("새 비밀번호 확인", type="password")
        if st.button("비밀번호 설정", type="primary", width="stretch"):
            if not (a.isdigit() and len(a) >= 4):
                st.error("4자리 이상의 숫자로 입력해 주세요.")
            elif a != b:
                st.error("두 비밀번호가 일치하지 않습니다.")
            else:
                setting_set("pin_hash", pin_hash(a))
